...
class LateralPlanner():
  ...
  def update(self, sm, CP, VM):
    ...
    #x = interp(v_ego, [10., 30.], [1.0, 0.9]) #max(sm['liveParameters'].stiffnessFactor, 0.1)
    self.VM_cnt += 1
    if self.VM_cnt % 1000 == 0:   # self.v_ego_ed < v_ego: # overHDtime 0.05sec
      tpmsPressureF = (sm['carState'].tpmsPressureFl + sm['carState'].tpmsPressureFr) / 2    
      x = interp(tpmsPressureF, [36., 43.], [0.6, 1.0]) # interp(v_ego, [10., 15], [1.1, 1.0]) * interp(tpmsPressureF, [36., 43.], [0.7, 1.0])
      print("!!!!!!!!! Tire Press", tpmsPressureF)
      sr = 14.4 #CP.steerRatio #max(sm['liveParameters'].steerRatio, 0.1)
...
#    self.v_ego_ed = v_ego
  ...
  def publish(self, sm, pm):
    ...
    plan_send.lateralPlan.steerRatio = 14.4 #self.steerRatio
    plan_send.lateralPlan.steerRateCost = 1.5 #ntune_get('steerRateCost')
    plan_send.lateralPlan.steerActuatorDelay = 0.1 #ntune_get('steerActuatorDelay')
